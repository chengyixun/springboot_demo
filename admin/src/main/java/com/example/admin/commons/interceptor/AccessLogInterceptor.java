...
public class AccessLogInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		log.info(String.format("Request[%s] URL:[%s],Protocol:[%s],Params:%s", uuid, request.getRequestURL(),
				request.getProtocol(), JSONObject.toJSONString(request.getParameterMap())));
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		log.info(String.format("Response[%s][%s] Timeout:[%s ms], ResponseStatus:[%s],ResponseBodySize:[%s],Error:[%s]",
				uuid, request.getRequestURL(), timeout, response.getStatus(), response.getBufferSize(),
