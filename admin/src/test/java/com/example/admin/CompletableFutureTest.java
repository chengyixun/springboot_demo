...
/**
 * @ClassName: CompletableFutureTest @Author: amy @Description:
 *             CompletableFutureTest @Date: 2021/12/14 @Version: 1.0
 */
...
public class CompletableFutureTest {
	...
	@Resource
	private Executor loggerTaskExecutor;
	...
	public void userTest1() {
		...
		CompletableFuture[] futures = users.stream().map(u -> disposeUser(u)).toArray(CompletableFuture[]::new);
	...
	public CompletableFuture<Integer> disposeUser(User user) {
		...
		return CompletableFuture.supplyAsync(new Supplier<Integer>() {
			...
			public Integer get() {
				...
				try {
					 ...
					 *
					 * <p>
					 * 大家在处理集合数据并且每一条的处理都比较耗时的话，可以考虑这个手法。
					 */
		...
		}, loggerTaskExecutor).handleAsync(new BiFunction<Integer, Throwable, Integer>() {
			@Override
