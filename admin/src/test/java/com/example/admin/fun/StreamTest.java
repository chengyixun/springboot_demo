...
/**
 * {@link StreamTest} stream: 1.不存储数据 2.不改变数据源，通常情况下会产生一个新的集合或一个值。
...
public class StreamTest {
	...
	public List<ScheduleBreak> scheduleBreaks() {
		...
		scheduleBreaks
				.add(ScheduleBreak.builder().unit("U_1").personId("xx").scheduleDate("2021-11-08").status(1).build());
		scheduleBreaks
				.add(ScheduleBreak.builder().unit("U_1").personId("yy").scheduleDate("2021-11-08").status(2).build());
		scheduleBreaks
				.add(ScheduleBreak.builder().unit("U_1").personId("zz").scheduleDate("2021-11-09").status(3).build());
		scheduleBreaks
				.add(ScheduleBreak.builder().unit("U_1").personId("cc").scheduleDate("2021-11-10").status(4).build());
		scheduleBreaks
				.add(ScheduleBreak.builder().unit("U_1").personId("aa").scheduleDate("2021-11-11").status(1).build());
	...
	public void testGroupBy() {
		...
		Map<String, Map<Integer, Long>> map = scheduleBreaks().stream()
				.collect(Collectors.groupingBy(ScheduleBreak::getScheduleDate,
	...
	/**
	 * 3. 遍历/匹配（foreach/find/match） stream和parallelStream的简单区分：
	...
	@Test
	public void test4() {
		List<String> names = personList().stream().filter(x -> x.getSalary() > 8000).map(Person::getName)
				.collect(Collectors.toList());
	 ...
	 *
	 * <p>
	 * map：接收一个函数作为参数，该函数会被应用到每个元素上，并将其映射到一个新的元素。
	 *
	 * <p>
	...
	/***
	 * mapToLong api 配合 boxed() 调用 LongStream.boxed 方法收集为 Stream<Long> 类型；
	 */
	@Test
	public void testMapToLong() {
		List<String> numbers = Arrays.asList("22", "19", "89", "90");
		List<Long> results = numbers.stream().mapToLong(Long::valueOf).boxed().collect(Collectors.toList());
	 ...
	 *
	 * <p>
	 * flatMap 适用于对数组流进行平铺后合并，两个方法的应用场景不一样。
	 */
	@Test
	...
	public void testFlatMap() {
		...
		List<String> collect = personList().stream().flatMap(m -> {
	...
	public void test10() {
		...
		//
		Map<String, List<Person>> group3 = personList().stream()
	...
	public void test12() {
		// 默认升序
		List<Person> list = personList().stream().sorted(Comparator.comparing(Person::getSalary))
				.collect(Collectors.toList());
		System.out.println("按工资升序排序：" + list);

		List<Person> list2 = personList().stream().sorted(Comparator.comparing(Person::getSalary).reversed())
				.collect(Collectors.toList());
	...
	public void testMap2() {
		...
		Map<String, String> stringMap = str.stream().collect(Collectors.toMap(Person::getName, Person::getArea));
	...
	public void testFlatMap3() {
		...
		Map<String, String> result = persons.stream()
				.filter(f -> org.apache.commons.collections4.CollectionUtils.isNotEmpty(f.getSupportList()))
				.flatMap(m -> m.getSupportList().stream().flatMap(s -> getSupports(s, m.getPersonId()).stream()))
				.collect(Collectors.toMap(k -> String.join(",", k.getPersonId(), k.getSupportDate()),
	...
	private static List<PersonSupportVO> getSupports(Support s, String personId) {
		...
		while (startLocalDate.compareTo(endLocalDate) <= 0) {
			PersonSupportVO personSupportVO = PersonSupportVO.builder().personId(personId)
					// DateKits.format(startLocalDate)
					.supportDate("ssss").supportUnitId(s.getSupportUnitId()).build();
	...
	public void testList() {
		...
		seconds.add(SecondChild.builder().name("s_name_0").code("s_code_0").ids(Lists.newArrayList("a", "b")).build());
		seconds.add(SecondChild.builder().name("s_name_1").code("s_code_1").ids(Lists.newArrayList("c", "d")).build());
		...
		List<String> ids = groups
				.stream().filter(f -> !CollectionUtils.isEmpty(f.getFirsts())).flatMap(g -> g.getFirsts().stream()
						.filter(f -> !CollectionUtils.isEmpty(f.getSeconds())).flatMap(s -> s.getSeconds().stream()
								.filter(f -> !CollectionUtils.isEmpty(f.getIds())).flatMap(o -> o.getIds().stream())))
				.collect(Collectors.toList());
		System.out.println(ids);

		Optional.ofNullable(groups).orElse(Collections.emptyList()).stream()
				.filter(f -> !CollectionUtils.isEmpty(f.getFirsts())).map(Group::getFirsts).flatMap(List::stream)
				.filter(c -> !CollectionUtils.isEmpty(c.getSeconds())).flatMap(f -> f.getSeconds().stream())
	...
	public void testNull() {
		...
		Map<String, Person> nameMap = Optional.ofNullable(persons).get().stream()
	...
	public void testSys() {
		...
		List<Person> persons = Lists.newArrayList();
		Optional<Person> personOptional = persons.stream().filter(p -> names.contains(p.getName())).findAny();
	...
	public void testSplice() {
		...
		persons.parallelStream().forEach(index -> {
	...
	public void testParallelStream() {
		...
		Map<Integer, User> userMap = users.stream().collect(Collectors.toMap(User::getAge, Function.identity()));

		// 先过滤一便，用parallelStream 处理 映射成新的 对象
		List<Employee> employees = persons.parallelStream().filter(f -> userMap.keySet().contains(f.getAge()))
				.map(m -> {
	...
	public void testSort3() {
		...
		// 未组装前的数据
		List<Blog> collect = blogs.stream()
				.sorted(Comparator.comparing(Blog::getCode).thenComparing(Blog::getCurrentCode))
				.collect(Collectors.toList());
		// 去重之后的数据
		Set<String> codes = blogs.stream().flatMap(f -> Stream.of(f.getCode(), f.getCurrentCode()))
		...
		List<String> currentCodes = blogs.stream().map(Blog::getCurrentCode).collect(Collectors.toList());

		codes.forEach(f -> {
			List<Blog> blogs1 = collect.stream()
					.filter(c -> f.equalsIgnoreCase(c.getCode()) || f.equalsIgnoreCase(c.getCurrentCode()))
					.collect(Collectors.toList());
			Set<String> collect1 = blogs1.stream().flatMap(f1 -> Stream.of(f1.getCode(), f1.getCurrentCode()))
					.collect(Collectors.toSet());
			collect1.remove(f);
	...
	public void testMapRefactor12() {
		List<Blog> blogs = blogs();
		Map<String, Long> longMap = blogs.stream().flatMap(f -> Stream.of(f.getCode(), f.getCurrentCode()))
		...
		blogs.stream()
				.filter(c -> CollectionUtils.isEmpty(result.get(c.getCode()))
	...
	public void testArrayToList() {
		...
		for (int[] ar : list) {
			...
			/**
			 * 由数组转换过来的集合，并不能添加或删除元素，但是可以用集合的其他方法
	...
	/**
	 * 1. Map 以前没有添加过相同的键，则put()和putIfAbsent()方法都返回的是null 2. putIfAbsent ->
	 * onlyIfAbsent 3. put
	 */
	@Test
	...
	public void testByFieldDistinct1() {
		List<Person> people = personList();
		List<Person> collect = people.stream().filter(distinctByKey(f -> f.getName())).collect(Collectors.toList());
	...
	public void testByFieldDistinct2() {
		List<Person> people = personList();
		List<Person> collect = people.stream().filter(distinctByKey2(f -> f.getName())).collect(Collectors.toList());
	...
	public void test90() {
		List<Person> persons = personList();
		Map<Integer, Set<Integer>> collect = persons.stream().collect(Collectors.groupingBy(Person::getAge,
	...
	@Test
	public void test91() {
		List<String> dates = Lists.newArrayList("2021-12-01", "2021-12-03", "2021-12-02", "2021-12-09", "2021-12-08");
