...
public class PageResult<E> implements Entity {
	...
	public void setPage(QueryParam queryParam) {
		...
		if (this.pageSize > 0) {
			this.totalPages = this.total % (long) this.pageSize == 0 ? this.total / (long) this.pageSize
					: this.total / (long) this.pageSize + 1L;
		}
	}
	...
	public void setTotal(long total) {
		this.total = total;
		if (this.pageSize > 0) {
			this.totalPages = this.total % (long) this.pageSize == 0L ? this.total / (long) this.pageSize
					: this.total / (long) this.pageSize + 1L;
		}
