...
/**
 * @ClassName: UserDetailServiceImpl @Author: amy @Description:
 *             loadUserByUsername 登陆用户名密码 @Date: 2021/7/6 @Version: 1.0
 */
@Component
	...
	public List<GrantedAuthority> getAuthority(User user) {
		...
		if (user.getIsSuperAdmin()) {
			...
			permissions = roles.stream().flatMap(role -> role.getMenus().stream())
					.filter(f -> StringUtils.isNotEmpty(f.getPermission())).map(Menu::getPermission)
