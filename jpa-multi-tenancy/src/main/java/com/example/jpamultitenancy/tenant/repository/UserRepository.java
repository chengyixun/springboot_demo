...
/**
 * @ClassName: UserRepository @Author: amy @Description: UserRepository @Date:
...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	// b.user_id
	@Query(value = "SELECT new com.example.jpamultitenancy.dto.UserBlogDTO(u.username,b.title) FROM User u LEFT JOIN Blog b ON u.id = b.user.id ")
	List<UserBlogDTO> selectUserBlogs();

	// nativeQuery= true 以sql语句执行
	// 错误 @Query(value = " SELECT id, username, password FROM sys_user WHERE age
	// =:age", nativeQuery =
	// true)
