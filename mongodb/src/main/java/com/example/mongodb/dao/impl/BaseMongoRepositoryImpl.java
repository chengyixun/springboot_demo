 ...
 * @date 5/31/21
 */
public abstract class BaseMongoRepositoryImpl<E extends BaseModel> implements BaseMongoRepository<E> {
	...
	public void updateOneSelective(E entity) {
		...
		mongoTemplate.findAndModify(Query.query(Criteria.where(DBCollection.ID_FIELD_NAME).is(entity.getId())), update,
	...
	public void removeByQuery(Query query, E entity) {
		mongoTemplate.remove(query, MongoHelper.getEntityClass(entity), MongoHelper.getCollectionName(entity));
	}
}
