...
/**
 * @ClassName: BaseMapperTest @Author: amy @Description: BaseMapperTest @Date: 2021/11/2 @Version:
 * 1.0
 */
...
public class BaseMapperTest {
   ...
   * <p>INSERT语句：没有影响
   *
   * <p>SELECT语句：追加WHERE条件，过滤掉已删除的数据
   *
   * <p>UPDATE语句：追加WHERE条件，防止更新到已删除的数据
   *
   * <p>DELETE语句：转变为UPDATE语句
   */
   ...
   * 1. 名字中包含佳，且年龄小于25
   *
   * <p>sql: SELECT * FROM biz_employee WHERE name like '%主%' AND age < 50
   *
   * <p>2. 名字为王姓，并且（年龄小于40，或者邮箱不为空）
   *
   * <p>sql: name like '王%' AND (age < 40 OR email is not null)
   *
   * <p>3. 满足：年龄为30，31，34，35, 返回满足条件的第一条记录
   *
   * <p>sql: ge IN (30,31,34,35) LIMIT 1
   *
   * <p>4. 只选出id, name 列 (QueryWrapper 特有)
   *
   * <p>sql: SELECT id, name FROM user;
   *
   * <p>5. 选出id, name, age, email, 等同于排除 manager_id 和 create_time // 当列特别多, 而只需要排除个别列时,
   * 采用上面的方式可能需要写很多个列, 可以采用重载的select方法，指定需要排除的列
   */
  @Test
   ...
   *
   * <p>UPDATE biz_employee SET email=? WHERE name LIKE CONCAT(?,'%')
   */
  @Test
   ...
   *
   * <p>若想要更新的只有那么一列，或者两列，则创建一个实体对象就显得有点麻烦。
   *
   * <p>针对这种情况，UpdateWrapper提供有set方法，可以手动拼接SQL中的SET语句，此时可以不必传入实体对象，示例如下
   */
  @Test
  ...
  /**
   * SELECT id,name,age,email,manager_id,create_time,version,deleted FROM biz_employee WHERE (age >
   * ?) LIMIT ?,?
   */
  @Test
  ...
  public void testAutoFillInsert() {
    Employee employee =
        Employee.builder().name("测试1").age(30).email("150121268@qq.com").managerId(2L).build();
   ...
   * 测试乐观锁
   *
   * <p>在读多写少的场景下，乐观锁比较适用，能够减少加锁操作导致的性能开销，提高系统吞吐量。
   *
   * <p>在写多读少的场景下，悲观锁比较使用，否则会因为乐观锁不断失败重试，反而导致性能下降。
   *
   * <p>乐观锁的实现如下： 取出记录时，获取当前version 更新时，带上这个version 执行更新时， set version = newVersion where version =
   * oldVersion 如果oldVersion与数据库中的version不一致，就更新失败 这种思想和CAS（Compare And Swap）非常相似。
   *
   * UPDATE biz_employee SET name=?, age=?, email=?, manager_id=?, create_time=?, update_time=?, version=? WHERE id=? AND version=? AND deleted=0
