...
/**
 * @ClassName: BaseServiceTest @Author: amy @Description: BaseServiceTest @Date:
...
public class BaseServiceTest {
	...
	public void test1() {
		...
		Employee one = employeeService.getOne(wrapper, false); // 第二参数指定为false,使得在查到了多行记录时,不抛出异常,而返回第一条记录

		log.info("getOne:{}", one);
	}

	/**
	 * IService也支持链式调用，代码写起来非常简洁
	...
	public void testLambdaQuery() {
		...
		List<Employee> list = employeeService.lambdaQuery().gt(Employee::getAge, 39).likeRight(Employee::getName, "王")
				.list();
	...
	public void testLambdaUpdate() {
		...
		employeeService.lambdaUpdate().ge(Employee::getName, "ee").set(Employee::getEmail, "xxx@123").update();
	}
